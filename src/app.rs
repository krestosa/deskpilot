...
pub fn run(data_dir: PathBuf, options: RunOptions) -> Result<(), String> {
    fs::create_dir_all(&data_dir).map_err(|error| format!("cannot create data directory {}: {error}", data_dir.display()))?;
    ...
    let (config, config_path) = Config::load_or_create(&data_dir).map_err(|error| error.to_string())?;
    let config = Arc::new(RwLock::new(config));
    let logging_config = config.read().map_err(|_| "configuration lock poisoned".to_string())?.logging.clone();
    ...
    let desktop_listener = if backend.compatible() {
        match winvd::listen_desktop_events(desktop_tx) {
            ...
            Err(error) => {
                log(LogLevel::Warn, format!("desktop event listener unavailable: {error:?}"));
                None
            }
        }
    } else {
    ...
    let mut state = AppState {
        ...
        hook_state: if options.no_hook { "disabled by --no-hook".to_string() } else { "active".to_string() },
    ...
    while !state.shutdown {
        ...
        let timeout = wait_until.saturating_duration_since(now).min(Duration::from_secs(1));
        ...
        match signal_rx.recv_timeout(timeout) {
            ...
            Ok(AppSignal::Ipc(request)) => {
                ...
                if reconcile { pending_reconcile = Some(Instant::now()); }
            }
        ...
        if now >= next_watchdog {
            ...
            next_watchdog = now + Duration::from_millis(config_snapshot.desktops.watchdog_interval_ms);
        }
    ...
    state.events.publish(Event::new("shutdown", "DeskPilot is shutting down"));
    drop(desktop_listener);
    if let Some(hook) = &mut hook { hook.stop(); }
    if let Some(tray) = &mut tray { tray.stop(); }
    drop(ipc);
...
impl AppState {
    fn config_read(&self) -> Config {
        self.config.read().map_or_else(|_| Config::default(), |config| config.clone())
    }

    fn save_config(&self, config: &Config) -> Result<(), String> {
        config.write_atomic(&self.config_path).map_err(|error| error.to_string())?;
        let mut target = self.config.write().map_err(|_| "configuration lock poisoned".to_string())?;
        *target = config.clone();
        Ok(())
    }
    ...
    fn navigate(&mut self, step: Step) {
        ...
        match self.backend.switch_relative(step, config.wheel.navigation) {
            Ok(desktop) => self.publish("desktop-switched", format!("desktop index {}", desktop.index)),
    ...
    fn reconcile(&mut self) {
        if !self.backend.compatible() { return; }
        ...
        for _ in 0..8 {
            ...
            for mutation in next.mutations {
                let result = match &mutation {
                    ...
                    Mutation::Remove { desktop, fallback } => self.backend.remove(desktop, fallback),
                };
                match result {
                    Ok(()) => mutations.push(mutation),
                    Err(error) => {
                        self.error(format!("reconciliation mutation failed: {mutation:?}: {error}"));
                        return;
                    }
                }
            }
    ...
    fn handle_tray(&mut self, command: TrayCommand, tray: Option<&Tray>) -> Result<bool, String> {
        ...
        match command {
            TrayCommand::ToggleEnabled => { config.enabled = !config.enabled; self.save_config(&config)?; }
            TrayCommand::ToggleDynamic => { config.desktops.dynamic = !config.desktops.dynamic; self.save_config(&config)?; reconcile = true; }
            TrayCommand::ToggleDirection => {
                config.wheel.direction = match config.wheel.direction { WheelDirection::Normal => WheelDirection::Inverted, WheelDirection::Inverted => WheelDirection::Normal };
                self.save_config(&config)?;
            }
            TrayCommand::ToggleNavigation => {
                config.wheel.navigation = match config.wheel.navigation { NavigationMode::Clamp => NavigationMode::Wrap, NavigationMode::Wrap => NavigationMode::Clamp };
                self.save_config(&config)?;
            }
            TrayCommand::Reconcile => reconcile = true,
            TrayCommand::Reload => { self.reload()?; reconcile = true; }
            ...
            TrayCommand::Diagnostics => {
                ...
                if let Ok(json) = serde_json::to_vec_pretty(&report) { let _ = fs::write(&path, json); open_path(&path); }
            }
            TrayCommand::ToggleStartup => {
                if startup::is_enabled() { startup::disable()?; } else { startup::enable(&std::env::current_exe().map_err(|error| error.to_string())?, &self.data_dir)?; }
            }
    ...
    fn handle_ipc(&mut self, request: ServerRequest) -> bool {
        ...
        let response = match command.as_str() {
            ...
            "desktops list" => self.backend.list().map_or_else(|error| IpcResponse::failure(69, error), |desktops| IpcResponse::success(desktops.iter().map(|desktop| json!({"id": desktop.id.0, "index": desktop.index})).collect::<Vec<_>>())),
            "desktops current" => self.backend.current().map_or_else(|error| IpcResponse::failure(69, error), |desktop| IpcResponse::success(json!({"id": desktop.id.0, "index": desktop.index}))),
            "desktops next" => self.backend.switch_relative(Step::Next, self.config_read().wheel.navigation).map_or_else(|error| IpcResponse::failure(69, error), |desktop| IpcResponse::success(json!({"id": desktop.id.0, "index": desktop.index}))),
            "desktops previous" => self.backend.switch_relative(Step::Previous, self.config_read().wheel.navigation).map_or_else(|error| IpcResponse::failure(69, error), |desktop| IpcResponse::success(json!({"id": desktop.id.0, "index": desktop.index}))),
            "desktops create" => self.backend.create().map_or_else(|error| IpcResponse::failure(69, error), |desktop| IpcResponse::success(json!({"id": desktop.id.0, "index": desktop.index}))),
            "reconcile" => { reconcile = true; IpcResponse::message("reconciliation scheduled") }
            "enable" | "disable" => {
                let mut config = self.config_read();
                config.enabled = command == "enable";
                self.save_config(&config).map_or_else(|error| IpcResponse::failure(78, error), |()| IpcResponse::success(json!({"enabled": config.enabled})))
            }
            "reload" => { reconcile = true; self.reload().map_or_else(|error| IpcResponse::failure(78, error), |()| IpcResponse::message("configuration reloaded")) }
            "config path" => IpcResponse::success(json!({"path": self.config_path})),
            "config show" => fs::read_to_string(&self.config_path).map_or_else(|error| IpcResponse::failure(74, error.to_string()), |text| IpcResponse::success(json!({"toml": text}))),
            "config validate" => Config::load(&self.config_path).map_or_else(|error| IpcResponse::failure(78, error.to_string()), |_| IpcResponse::message("configuration valid")),
            "logs path" => IpcResponse::success(json!({"path": self.data_dir.join("logs")})),
            "logs tail" => IpcResponse::success(json!({"lines": tail_log(&self.data_dir.join("logs").join("deskpilot.log"), 100)})),
            "support-bundle" => {
                ...
                create_support_bundle(&self.data_dir, &doctor, &redacted)
                    .map_or_else(|error| IpcResponse::failure(74, error), |path| IpcResponse::success(json!({"path": path})))
            }
            "startup enable" => startup::enable(&std::env::current_exe().unwrap_or_default(), &self.data_dir).map_or_else(|error| IpcResponse::failure(74, error), |()| IpcResponse::message("startup enabled")),
            "startup disable" => startup::disable().map_or_else(|error| IpcResponse::failure(74, error), |()| IpcResponse::message("startup disabled")),
            "shutdown" => { self.shutdown = true; IpcResponse::message("shutdown requested") }
    ...
    fn doctor(&mut self) -> DoctorReport {
        ...
        let occupancy = snapshot.as_ref().map_or(OccupancyDiagnostic { occupied: 0, empty: 0, unknown: 0 }, |states| OccupancyDiagnostic {
            occupied: states.iter().filter(|state| state.occupancy == Occupancy::Occupied).count(),
            empty: states.iter().filter(|state| state.occupancy == Occupancy::Empty).count(),
            unknown: states.iter().filter(|state| state.occupancy == Occupancy::Unknown).count(),
        });
        ...
        DoctorReport {
            ...
            configuration_status: if config.validate().is_ok() { "valid".to_string() } else { "invalid".to_string() },
            ...
            backend: BackendDiagnostic {
                ...
                ].into_iter().filter_map(|(enabled, name)| enabled.then_some(name.to_string())).collect(),
                error: (!self.backend.compatible()).then(|| self.backend.compatibility_reason().to_string()),
            },
            ...
            dynamic_reconciliation: if config.desktops.dynamic && !self.dynamic_forced_off { "enabled" } else { "disabled" }.to_string(),
    ...
    fn publish(&self, kind: &str, message: impl Into<String>) {
        ...
        if self.foreground { println!("{} {kind}: {message}", timestamp_utc()); }
        self.events.publish(Event::new(kind, message));
    }

    fn error(&self, message: String) {
        log(LogLevel::Error, &message);
        if self.foreground { eprintln!("{} error: {message}", timestamp_utc()); }
    ...
    F: Fn(T) -> AppSignal + Send + 'static,
{
    thread::spawn(move || {
        for value in receiver {
            if sender.send(map(value)).is_err() { break; }
        }
    });
}
...
fn open_path(path: &Path) {
    ...
    unsafe {
        ShellExecuteW(0, operation.as_ptr(), target.as_ptr(), std::ptr::null(), std::ptr::null(), SW_SHOWNORMAL);
    }
}

fn tail_log(path: &Path, lines: usize) -> Vec<String> {
    fs::read_to_string(path)
        .map(|text| text.lines().rev().take(lines).collect::<Vec<_>>().into_iter().rev().map(str::to_string).collect())
        .unwrap_or_default()
}
...
fn install_panic_hook(data_dir: &Path) {
    ...
    std::panic::set_hook(Box::new(move |info| {
        let path = crash_dir.join(format!("crash-{}.txt", timestamp_utc().replace([':', '.'], "-")));
        let message = format!("DeskPilot {}\n{}\n", APP_VERSION, info);
        let _ = fs::write(path, message.as_bytes().get(..16 * 1024).unwrap_or(message.as_bytes()));
    }));
}
...
impl InstanceGuard {
    fn acquire() -> Result<Self, String> {
        ...
        unsafe {
            ...
            if handle == 0 { return Err(format!("CreateMutexW failed: {}", GetLastError())); }
            Ok(Self { handle, already_exists: GetLastError() == ERROR_ALREADY_EXISTS })
        }
    }
}
