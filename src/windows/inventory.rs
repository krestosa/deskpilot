...
use windows_sys::Win32::UI::WindowsAndMessaging::{
    EnumWindows, GetClassNameW, GetForegroundWindow, GetMonitorInfoW, GetWindow,
    GetWindowLongPtrW, GetWindowRect, GetWindowThreadProcessId, IsWindow, IsWindowVisible,
    MonitorFromWindow, GWL_EXSTYLE, GW_OWNER, MONITORINFO, MONITOR_DEFAULTTONEAREST,
...
pub fn snapshot(backend: &WinvdBackend, config: &Config, grace: &HashMap<DesktopId, bool>) -> Result<Vec<DesktopState>, String> {
    ...
    for hwnd in enumerate_windows() {
        let Some(basic) = inspect_basic(hwnd) else { continue };
        ...
        if !occupancy.contains_key(&desktop) { continue; }
        if config.windows.ignore_classes.iter().any(|value| value.eq_ignore_ascii_case(&basic.class_name)) {
            continue;
        }
        ...
        match executable_name(basic.process_id) {
            Ok(executable) => {
                if config.windows.ignore_executables.iter().any(|value| value.eq_ignore_ascii_case(&executable)) {
                    continue;
                }
...
pub fn exclusive_fullscreen_active() -> bool {
    unsafe {
        ...
        let mut info = MONITORINFO { cbSize: size_of::<MONITORINFO>() as u32, rcMonitor: RECT::default(), rcWork: RECT::default(), dwFlags: 0 };
        if GetMonitorInfoW(monitor, &mut info) == 0 { return false; }
...
fn inspect_basic(hwnd: HWND) -> Option<BasicWindow> {
    unsafe {
        ...
        if ex_style & (WS_EX_TOOLWINDOW | WS_EX_NOACTIVATE) != 0 { return None; }
        ...
        Some(BasicWindow { class_name: from_wide(&class[..length as usize]), process_id })
    }
}
...
fn executable_name(process_id: u32) -> Result<String, String> {
    unsafe {
        ...
        if process == 0 { return Err("OpenProcess failed".to_string()); }
        let mut buffer = vec![0_u16; 32_768];
        let length = K32GetModuleFileNameExW(process, 0, buffer.as_mut_ptr(), buffer.len() as u32);
        CloseHandle(process);
        if length == 0 { return Err("K32GetModuleFileNameExW failed".to_string()); }
...
fn ignored_class(class_name: &str) -> bool {
    ...
    CLASSES.iter().any(|value| value.eq_ignore_ascii_case(class_name))
}
